...
for item in dirs:
			...
			buffer = info['name'] + "\r\n"
			buffer += "In folder: " + folder + "\r\n"
			buffer += "(" + info['bundleid'] + ") by " + info['createdby'] + "\r\n"
			
			fulldir = os.system("pwd")
			write = "echo " + buffer + " >> " + fulldir + "log.txt"
			os.system(write)
			...
			if info['description']: # Is the description present? Some people don't include these...
				buffer +=  "Description: " + info['description'] + "\r\n"
			else:
			...
			for item in info['objects']:
				if item['type'] == "alfred.workflow.input.keyword":		# Keywords
					buffer += "-- alfred.workflow.input.keyword: "
					if "keyword" in item['config']:
						buffer += item['config']['keyword'] + "\r\n"
					else:
						buffer += "<font color=\"red\">No Keyword in Keyword Workflow Entry</font>\r\n"
					if "text" in item['config']:
						buffer += "Text: " + item['config']['text'] + "\r\n"
					else:
						buffer += "No Text has been entered to describe the keyword.\r\n"						
					if "subtext" in item['config']:
						buffer += "Subtext: " + item['config']['subtext'] + "\r\n"
					else:
						buffer += "No Subtext has been entered to describe the keyword.\r\n"
					buffer += "---\r\n\r\n"
				if item['type'] == "alfred.workflow.trigger.hotkey":	# Hotkeys
					buffer += "-- alfred.workflow.trigger.hotkey: "
					if "hotmod" in item['config'] and item['config']['hotmod']:
						buffer += " hotmod key exists | "
						if item['config']['hotmod'] in hotmod:
							buffer += hotmod[item['config']['hotmod']] + " " + item['config']['hotstring']
						else:
							buffer += "<font color=\"red\">No Hotkeys Defined</font>"
					else:
