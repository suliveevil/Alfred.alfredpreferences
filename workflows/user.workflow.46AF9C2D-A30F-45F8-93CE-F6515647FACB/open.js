...
var Opener = function (_Action) {
    babelHelpers.inherits(Opener, _Action);

    function Opener() {
        babelHelpers.classCallCheck(this, Opener);
        return babelHelpers.possibleConstructorReturn(this, Object.getPrototypeOf(Opener).apply(this, arguments));
    }

    babelHelpers.createClass(Opener, [{
        key: 'preview',
        value: function preview() {
            var _this2 = this;
            ...
            var xml = this.previewOptionSelects(['browsers'], preview, options, ['in']);
            if (xml) {
                return xml;
            }
            ...
            try {
                ...
                var _app3 = babelHelpers.slicedToArray(_app2, 3);

                var app = _app3[0];
                var /*windows*/browserType = _app3[2];

                appName = app.name();
                options.in = options.in || getBrowser(browserType, null, getDefaultBrowser())[0];
            } catch (err) {
                console.log(err);
                var opt = appName ? null : 'in';
                return this.previewOptionSelectsError(err, preview, opt);
            }

            var text = getAppData(appName, ['selection'], { source: _app }).selection || theClipboard();
            var urls = validateUrl(text, true);

            var _getBrowser = getBrowser(options.in);

            var _getBrowser2 = babelHelpers.slicedToArray(_getBrowser, 2);

            var inBrowser = _getBrowser2[0];
            var inBrowserType = _getBrowser2[1];


            if (urls.length) {
                urls.forEach(function (url) {
                    var query = _this2.constructQueryString({ notes: url });
                    var item = {
                        arg: query,
                        //autocomplete: query,
                        title: url,
        ...
        value: function run() {
            ...
            var search = _getQueryFlags.search;

            var options = this.getQueryOptions({ allowEmpty: false, sanitizer: true });

            var notes = this.getQueryNotes();
            notes = notes || getAppData(undefined, ['selection']).selection || theClipboard();

            var _openUrl = openUrl(notes, options.in, { dedupe: dedupe, fallbackSearch: search });

            var _openUrl2 = babelHelpers.slicedToArray(_openUrl, 2);

            var url = _openUrl2[0];
            var appName = _openUrl2[1];
...
var action = new Opener({
    name: 'open',
    title: 'Smart Opener',
    // opt: [ name, test, default, required, sanitizer]
    opts: [['in', 'Target browser to open URL(s) in', 1]],
    // flag: [ name, test, default]
    flags: [['dedupe', 'Deduplicate URLs in target browser before open new tab', 1, true], ['search', 'Search text instead', 1, true]]
});
