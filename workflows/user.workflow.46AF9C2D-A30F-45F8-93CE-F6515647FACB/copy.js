...
var FORMATS_FILE$1 = 'formats.json';

var Copier = function (_Action) {
    babelHelpers.inherits(Copier, _Action);

    function Copier() {
        babelHelpers.classCallCheck(this, Copier);
        return babelHelpers.possibleConstructorReturn(this, Object.getPrototypeOf(Copier).apply(this, arguments));
    }

    babelHelpers.createClass(Copier, [{
        key: 'preview',
        value: function preview() {
            var _this2 = this;
            ...
            var xml = this.previewOptionSelects(['frontmost', 'browsers'], preview, options, ['from']);
            if (xml) {
                return xml;
            }
            ...
            var _app3 = babelHelpers.slicedToArray(_app2, 7);

            var app = _app3[0];
            var windows = _app3[1];
            var browserType = _app3[2];
            var targetTab = _app3[3];
            var targetTabIndex = _app3[4];
            var title = _app3[5];
            var url = _app3[6];
            ...
            if (browserType) {
                var _ret = function () {
                    ...
                    if (!tabs || !tabs.length) {
                        return {
                            v: _this2.previewOptionSelectsError('No active tab in ' + appName, preview, 'from')
                        };
                    }
                    ...
                    tabs.forEach(function (_ref) {
                        ...
                        item.title = '✄ ' + item.title, preview.add(item, active ? 0 : 999);
                    });
                }();
            ...
            // Other types
                    ...
                    var query = this.constructQueryString({
        ...
        value: function run() {
            ...
            var format = _getQueryOptions.format;

            var clips = this.getQueryFlags();
            ...
            if (format) {
                var formats = void 0;
                try {
                    formats = JSON.parse(readFromFile(FORMATS_FILE$1));
                } catch (err) {
                    console.log(err);
                }
            ...
            if (index === 'all') {
                var _ret2 = function () {
                    ...
                    var extraClips = new Set(['selection', 'markdown'].filter(function (f) {
                        return clips.has(f);
                    }));
                    clips = Array.from(clips);

                    var _getAppData = getAppData(from, ['tabs']);

                    var appName = _getAppData.appName;
                    var tabs = _getAppData.tabs;

                    tabs = tabs || [];
                    var text = tabs.map(function (_ref2) {
                        var url = _ref2.url;
                        ...
                        if (extraClips.size) {
                            var extraData = getAppData(appName, extraClips, { index: index });
                            data = babelHelpers.extends(data, extraData);
                        }

                        return _this3.formatText(formatString, clips, data);
                    }).join('\n');

                    theClipboard(text);
                    return {
                        v: 'Copied ' + clips.join(',').toUpperCase() + ' from all tabs in ' + appName
                    };
                }();
                ...
                clips = Array.from(clips);
                var data = getAppData(from, clips, { index: index, stringify: true });
                var _text = this.formatText(formatString, clips, data);
        ...
        key: 'formatText',
        value: function formatText(formatString) {
            var clips = arguments.length <= 1 || arguments[1] === undefined ? [] : arguments[1];
            var data = arguments.length <= 2 || arguments[2] === undefined ? {} : arguments[2];

            if (formatString) {
                var re = /##[\w]+##/ig;
                return formatString.replace(/##[\w]+##/g, function (m) {
                    m = m.slice(2, -2);
                    return data[m] || '';
                });
            } else {
                return clips.map(function (type) {
                    return data[type] || '';
                }).join('\n');
            }
        }
...
var action = new Copier({
    name: 'copy',
    title: 'Super Copier',
    // opt: [ name, test, default, required, sanitizer]
    opts: [['from', 'Source application to copy data from', 1], ['index', 'Tab index number if available', 1, null, false, ['all', Number.parseInt]], ['format', 'Preset text style when constructing copied data', 2]],
    // flag: [ name, test, default]
    flags: [['url', 'Copy URL', 1], ['title', 'Copy tab title or application name', 2], ['selection', 'Copy text of selection', 1], ['tabs', 'Copy tabs data', 2], ['markdown', 'Copy URL and title in Markdown format', 1]]
});
