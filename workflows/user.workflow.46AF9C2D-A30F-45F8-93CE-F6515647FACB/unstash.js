...
var STASH_FILE = 'stash.json';

var UnStash = function (_Action) {
    babelHelpers.inherits(UnStash, _Action);

    function UnStash() {
        babelHelpers.classCallCheck(this, UnStash);
        return babelHelpers.possibleConstructorReturn(this, Object.getPrototypeOf(UnStash).apply(this, arguments));
    }

    babelHelpers.createClass(UnStash, [{
        key: 'preview',
        value: function preview() {
            var _this2 = this;
            ...
            var preview = new Preview();
            var options = this.getQueryOptions();
            var notes = this.getQueryNotes();

            var xml = this.previewOptionSelects(['browsers'], preview, options, ['to']);
            if (xml) {
                return xml;
            }

            var toBrowser = void 0;
            try {
            ...
            stashList.forEach(function (_ref, i) {
                var name = _ref.name;
                ...
                if (notes) {
                    var tester = getTester(name, 'words_i');
                    if (!tester(notes)) {
                        return;
                    }
                }
                ...
                var text = tabs.map(function (_ref2) {
                    ...
                    var url = _ref3[0];
                    var title = _ref3[1];
                    return '[' + decodeURIComponent(title) + '](' + decodeURIComponent(url) + ')';
                }).join('\n');
            ...
            if (!preview.length()) {
                ...
                var title = stashList.length ? 'No matched stash was found for \'' + notes + '\'' : 'No stash was saved.';
        ...
        value: function run() {
            ...
            var newwindow = _getQueryFlags.newwindow;

            var _getQueryOptions = this.getQueryOptions({ allowEmpty: false, sanitize: true });

            var to = _getQueryOptions.to;
            var _getQueryOptions$inde = _getQueryOptions.index;
            var index = _getQueryOptions$inde === undefined ? 0 : _getQueryOptions$inde;
            ...
            var stash = stashList[index];
            if (!stash) {
                throw new Error('Invalid stash index number - ' + index);
            }

            var name = stash.name;
            var appName = stash.appName;
            var timestamp = stash.timestamp;
            var tabs = stash.tabs;

            var urls = [];
            var target = toBrowser || appName;
            var sameBrowser = target === appName;

            tabs.forEach(function (_ref4) {
                var _ref5 = babelHelpers.slicedToArray(_ref4, 1);
...
var action = new UnStash({
    name: 'unstash',
    title: 'Un-Stash Tabs',
    // opt: [ name, test, default, required, sanitizer, description]
    opts: [['to', 'Target browser to unstash tabs to.', 1], ['index', 'Tab index number', 1, null, false, Number.parseInt]],
    // flag: [ name, test, default, description]
    flags: [['clone', 'Do not delete stash record after unstashed', 1, false], ['newwindow', 'Unstash tabs into new window', 1, true], ['dedupe', 'Deduplicate URLs when openning in target window', 1]]
});
