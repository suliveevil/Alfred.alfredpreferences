...
var Stash = function (_Action) {
    babelHelpers.inherits(Stash, _Action);

    function Stash() {
        babelHelpers.classCallCheck(this, Stash);
        return babelHelpers.possibleConstructorReturn(this, Object.getPrototypeOf(Stash).apply(this, arguments));
    }

    babelHelpers.createClass(Stash, [{
        key: 'preview',
        value: function preview() {
            var _this2 = this;
            ...
            var xml = this.previewOptionSelects(['browsers'], preview, options, ['from']);
            if (xml) {
                return xml;
            }
            ...
            try {
                var _getFromToBrowsers = getFromToBrowsers(options.from);

                var _getFromToBrowsers2 = babelHelpers.slicedToArray(_getFromToBrowsers, 1);
            ...
            var _getAppData = getAppData(fromBrowser, ['tabs', 'browserType']);

            var tabs = _getAppData.tabs;
            var browserType = _getAppData.browserType;

            if (!tabs || !tabs.length) {
                return this.previewOptionSelectsError('No active tab in ' + fromBrowser, preview, 'from');
            }

            tabs.forEach(function (_ref) {
                var /*browser, browserType, */url = _ref.url;
                ...
                var subtitle = 'Stash all tabs to group "' + (notes || 'Untitled') + '": ' + url;
        ...
        value: function run() {
            ...
            var forever = _getQueryFlags2.forever;
            ...
            var _getFromToBrowsers3 = getFromToBrowsers(from);

            var _getFromToBrowsers4 = babelHelpers.slicedToArray(_getFromToBrowsers3, 1);
            ...
            if (!tabs || !tabs.length) {
                throw new Error('No tab in front window of ' + browser + '.');
            }

            // Encode non-latin characters to avoid error when writing to file
            tabs = tabs.map(function (_ref2) {
                var url = _ref2.url;
                var title = _ref2.title;
                return [encodeURIComponent(url), encodeURIComponent(title)];
            });
            var name = encodeURIComponent(notes || tabs[0][1] || 'Untitled Stash');
            var stash = {
                name: name,
                appName: fromBrowser,
...
var action = new Stash({
    name: 'stash',
    title: 'Stash Tabs',
    // opt: [ name, test, default, required, sanitizer]
    opts: [['from', 'Source browser to get list of tabs to stash', 1]],
    // flag: [ name, test, default]
    flags: [['clone', 'Do not close tabs after saved to stash', 1]]
});
