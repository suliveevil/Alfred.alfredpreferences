...
{
	private $_url = "http://openapi.youdao.com/api"; //?keyfrom=$from&key=$key&type=data&doctype=json&version=1.1&q=$q"
	private $_query = null;
	...
	public function __construct($from, $key, $q)
	{
		...
		$str = '73e2b53f1acb542c' . $q . '2222yzFM70b3hm2VHLW9UVF8AA5zPliL303u';
		$sign = md5($str);
		$this->_url .= '?' . http_build_query(array(
			'from'	=> 'auto',
			'appKey'		=> '73e2b53f1acb542c',
			'to'		=> 'auto',
			'salt'	=> 2222,
			'sign'	=> $sign,
			'q'			=> $q,
